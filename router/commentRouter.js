...
const { isLoggedIn, asyncError, validateComment, validateReply } = require("../middleware");

router.post("/", isLoggedIn, validateComment, asyncError(async (req, res) => {
...
router.post("/:ID/reply", isLoggedIn, validateReply, asyncError(async (req, res) => {
    const post = await Post.findById(req.params.id);
